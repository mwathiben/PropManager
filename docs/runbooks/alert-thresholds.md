...
| `legal_hold_stale_count` | sev3 > 0 | `legal-hold:sweep-stale` daily 05:10 EAT (Phase-68 STALE-SWEEP) → `legal_hold_stale` alert | instantaneous | Holds active past `legal_hold.stale_after_days` (default 365). The sweeper emails the owning landlord (cooldown `stale_reminder_cooldown_days`, default 30) but never deletes. Sustained count = holds nobody is releasing after litigation resolved. Phase-68 STALE-SWEEP / sev3 email. | Per `docs/runbooks/legal-hold.md#stale-holds`: confirm with the landlord whether each hold is still required; release the resolved ones; the count drops as holds are released. |
| `legal_hold_blocked_deletions_count` | visibility-only | `LegalHoldActiveException` render handler in `bootstrap/app.php` (Phase-68 HOLD-GUARD-2) | rolling 24h | Increments whenever someone attempts to delete a held subject (label = subject_type). A spike means users are repeatedly hitting held records — possibly unaware of an active hold, or a stale hold that should be released. Phase-68 HOLD-GUARD-2 / visibility-only. | If concentrated on one subject, check whether the hold is still needed (`/legal-holds`); the deletion is correctly blocked either way. |
| `legal_holds_blocking_erasure` | visibility-only | `DataDeletionService::executeDeletion` carve-out (Phase-68 HOLD-GUARD-1) | per-erasure | A GDPR Art. 17 erasure preserved N documents under active hold (Art. 17(3)(b) carve-out) rather than deleting them. The erasure still completes for everything else. Phase-68 HOLD-GUARD-1 / visibility-only. | Expected when an erasure target has held documents; once the hold is released a later erasure pass (or manual cleanup) removes them. |
