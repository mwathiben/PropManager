...
class Phase24BackendTest extends TestCase
{
    ...
    {
        // I18N-BACKEND-3: the email channel dispatches via Mail::to($recipient)
        ...
        // App::getLocale() points elsewhere. (M2: the email transport was
        // extracted from NotificationService into ChannelTransport.)
        $source = file_get_contents(app_path('Services/Notification/ChannelTransport.php'));

        $this->assertStringContainsString(
            'Mail::to($recipient)->send(new NotificationMail(',
            $source,
            'I18N-BACKEND-3: ChannelTransport must pass the User model to Mail::to() so HasLocalePreference is honoured.',
        );
        $this->assertStringNotContainsString(
            'Mail::to($recipient->email)->send(new NotificationMail(',
            $source,
            'I18N-BACKEND-3: ChannelTransport must NOT pass the email string — that bypasses HasLocalePreference.',
        );
    }
