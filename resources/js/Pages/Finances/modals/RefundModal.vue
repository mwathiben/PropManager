...
const refundMethods = computed(() => [
    { id: 'original', label: t('finances_refund.methods.original') },
    { id: 'cash', label: t('finances_refund.methods.cash') },
    { id: 'bank_transfer', label: t('finances_refund.methods.bank_transfer') },
    { id: 'mobile_money', label: t('finances_refund.methods.mobile_money') },
]);

const refundReasons = computed(() => [
    { value: 'Overpayment', label: t('finances_refund.reasons.overpayment') },
    { value: 'Duplicate Payment', label: t('finances_refund.reasons.duplicate') },
    { value: 'Tenant Moved Out', label: t('finances_refund.reasons.moved_out') },
    { value: 'Billing Error', label: t('finances_refund.reasons.billing_error') },
    { value: 'Service Not Provided', label: t('finances_refund.reasons.service_not_provided') },
    { value: 'Other', label: t('finances_refund.reasons.other') },
]);
...
const validate = () => {
    ...
    if (!form.value.payment_id) {
        errors.value.payment_id = t('finances_refund.errors.select_payment');
    }

    if (!form.value.amount || form.value.amount <= 0) {
        errors.value.amount = t('finances_refund.errors.valid_amount');
    } else if (form.value.amount > maxAmount.value) {
        errors.value.amount = t('finances_refund.errors.amount_exceeds', { max: formatMoney(maxAmount.value) });
    }

    if (!form.value.reason) {
        errors.value.reason = t('finances_refund.errors.select_reason');
    }

    if (!form.value.refund_method) {
        errors.value.refund_method = t('finances_refund.errors.select_method');
    }
...
const handleSubmit = async () => {
    ...
    try {
        ...
        errors.value.general = refundError.value || t('finances_refund.errors.failed');
    }
};
...
<template>
    <Modal :show="modalData.show" max-width="md" @close="close">
                        <div v-if="success" class="p-8 text-center">
                            ...
                            <h3 class="text-lg font-semibold text-gray-900">{{ t('finances_refund.success_title') }}</h3>
                            <p class="text-sm text-gray-500 mt-2">{{ t('finances_refund.success_body') }}</p>
                        </div>
                        ...
                        <template v-else>
                            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                                <div class="flex items-center gap-3">
                                    ...
                                    <h2 class="text-lg font-semibold text-gray-900">{{ t('finances_refund.heading') }}</h2>
                                </div>
                                <button
                                    @click="close"
                            ...
                            <form @submit.prevent="handleSubmit" class="p-6 space-y-4">
                                ...
                                <div>
                                    ...
                                    >
                                        <option :value="null">{{ t('finances_refund.select_payment') }}</option>
                                        <option
                                        ...
                                        >
                                            ...
                                            {{ payment.refund_status === 'refunded' ? t('finances_refund.already_refunded') : '' }}
                                        </option>
                                    </select>
                                ...
                                <div v-if="selectedPayment" class="p-3 bg-gray-50 rounded-lg text-sm">
                                    <div class="flex justify-between mb-1">
                                        <span class="text-gray-500">{{ t('finances_refund.original_amount') }}</span>
                                        <span class="font-medium text-gray-900">{{ formatMoney(selectedPayment.amount) }}</span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span class="text-gray-500">{{ t('finances_refund.payment_method') }}</span>
                                ...
                                <div>
                                    ...
                                    >
                                        <option value="">{{ t('finances_refund.select_reason') }}</option>
                                ...
                                <div class="flex gap-3 pt-2">
                                    ...
                                    >
                                        {{ isProcessing ? t('finances_refund.processing') : t('finances_refund.heading') }}
                                    </button>
                                </div>
                            </form>
                        </template>
