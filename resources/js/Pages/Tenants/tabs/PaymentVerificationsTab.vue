        ...
        <div class="bg-gray-50 rounded-lg p-4 mb-6 border border-gray-200">
            <div class="flex flex-wrap gap-4">
                <div class="flex-1 min-w-[200px]">
                    <div class="relative">
                        <input v-model="search" @keyup.enter="applyFilters" type="text" placeholder="Search payment verifications..."
        ...
        <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
            <table v-if="paymentVerifications?.data?.length" class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-end text-xs font-medium text-gray-500 uppercase tracking-wider">Amount</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="pv in paymentVerifications.data" :key="pv.id" class="hover:bg-gray-50">
                        <td class="px-6 py-4">
                            <div class="text-sm font-medium text-gray-900">{{ pv.tenant?.name || 'Unknown' }}</div>
                            <div class="text-xs text-gray-500">{{ pv.tenant?.email }}</div>
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ pv.unit?.unit_number ? `Unit ${pv.unit.unit_number}` : '—' }}</td>
                        <td class="px-6 py-4 text-sm text-gray-900 text-end">{{ pv.amount != null ? formatCurrency(pv.amount) : '—' }}</td>
                        <td class="px-6 py-4 text-center"><span :class="[statusColor(pv.status), 'px-2 py-1 text-xs font-medium rounded-full']">{{ pv.status }}</span></td>
                        <td class="px-6 py-4 text-end">
                            <Link :href="route('payment-verifications.show', pv.id)" class="text-gray-600 hover:text-gray-900" title="View"><EyeIcon class="w-5 h-5" /></Link>
                        </td>
                    </tr>
                </tbody>
            </table>

            <EmptyState v-else :icon="BanknotesIcon" title="No payment verifications"
                :description="hasActiveFilters ? 'Try adjusting your filters.' : 'Payment verifications will appear here.'" />
            ...
            <div v-if="paymentVerifications?.data?.length && paymentVerifications.last_page > 1" class="bg-gray-50 px-4 py-3 border-t border-gray-200">
                <div class="flex items-center justify-between">
                    <div class="text-sm text-gray-700">Showing {{ paymentVerifications.from }} to {{ paymentVerifications.to }} of {{ paymentVerifications.total }} results</div>
