...
<template>
    <div>
        <div v-if="stats" class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div class="bg-white rounded-xl border border-gray-200 p-4"><p class="text-2xl font-semibold text-gray-900">{{ stats.active ?? 0 }}</p><p class="text-sm text-gray-500">Active</p></div>
            <div class="bg-white rounded-xl border border-gray-200 p-4"><p class="text-2xl font-semibold text-gray-900">{{ stats.inspection_pending ?? 0 }}</p><p class="text-sm text-gray-500">Inspection pending</p></div>
            <div class="bg-white rounded-xl border border-gray-200 p-4"><p class="text-2xl font-semibold text-gray-900">{{ stats.settlement_pending ?? 0 }}</p><p class="text-sm text-gray-500">Settlement pending</p></div>
            <div class="bg-white rounded-xl border border-gray-200 p-4"><p class="text-2xl font-semibold text-gray-900">{{ stats.completed_this_month ?? 0 }}</p><p class="text-sm text-gray-500">Completed (month)</p></div>
        </div>
            ...
            <table v-if="moveOuts?.data?.length" class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase tracking-wider">Initiated</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="mo in moveOuts.data" :key="mo.id" class="hover:bg-gray-50">
                        <td class="px-6 py-4">
                            <div class="text-sm font-medium text-gray-900">{{ mo.lease?.tenant?.name || 'Unknown' }}</div>
                            <div class="text-xs text-gray-500">{{ mo.lease?.tenant?.email }}</div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="text-sm text-gray-900">Unit {{ mo.lease?.unit?.unit_number || '—' }}</div>
                        ...
                        <td class="px-6 py-4 text-end">
                            <Link :href="route('move-outs.show', mo.id)" class="text-gray-600 hover:text-gray-900" title="View"><EyeIcon class="w-5 h-5" /></Link>
                        </td>
                    </tr>
                </tbody>
            </table>

            <EmptyState v-else :icon="ArrowRightOnRectangleIcon" title="No move-outs"
                description="Move-out cases will appear here." />

            <div v-if="moveOuts?.data?.length && moveOuts.last_page > 1" class="bg-gray-50 px-4 py-3 border-t border-gray-200">
                <div class="flex items-center justify-between">
                    <div class="text-sm text-gray-700">Showing {{ moveOuts.from }} to {{ moveOuts.to }} of {{ moveOuts.total }} results</div>
