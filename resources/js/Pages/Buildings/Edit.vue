...
const tabs = computed(() => [
    { id: 'units', name: t('buildings_edit.tabs.units'), icon: Square2StackIcon },
    { id: 'settings', name: t('buildings_edit.tabs.settings'), icon: BuildingOfficeIcon },
    { id: 'amenities', name: t('buildings_edit.tabs.amenities'), icon: SparklesIcon },
    { id: 'location', name: t('buildings_edit.tabs.location'), icon: MapPinIcon },
    { id: 'automation', name: t('buildings_edit.tabs.automation'), icon: ClockIcon },
    { id: 'deductions', name: t('buildings_edit.tabs.deductions'), icon: DocumentTextIcon },
]);
...
const categoryConfig = computed(() => ({
    utilities: { icon: BoltIcon, label: t('buildings_edit.category.utilities') },
    security: { icon: ShieldCheckIcon, label: t('buildings_edit.category.security') },
    parking: { icon: TruckIcon, label: t('buildings_edit.category.parking') },
    common_amenities: { icon: SparklesIcon, label: t('buildings_edit.category.common_amenities') },
    unit_features: { icon: HomeModernIcon, label: t('buildings_edit.category.unit_features') },
    neighborhood: { icon: MapIcon, label: t('buildings_edit.category.neighborhood') },
}));
...
const buildingTypes = computed(() => [
    { value: 'residential_apartment', label: t('buildings_edit.building_type.residential_apartment'), icon: HomeIcon },
    { value: 'office_block', label: t('buildings_edit.building_type.office_block'), icon: BuildingOfficeIcon },
    { value: 'mixed_use', label: t('buildings_edit.building_type.mixed_use'), icon: BuildingStorefrontIcon },
    { value: 'single_unit_rental', label: t('buildings_edit.building_type.single_unit_rental'), icon: HomeModernIcon },
    { value: 'warehouse', label: t('buildings_edit.building_type.warehouse'), icon: TruckIcon },
]);
...
<template>
    <Head :title="t('buildings_edit.page_title')" />
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500">{{ building.property?.name }} &middot; {{ t('buildings_edit.header.units_count', { count: units.length }) }}</p>
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    <Link :href="route('buildings.dashboard', building.id)"
                        ...
                        {{ t('buildings_edit.header.view_dashboard') }}
                    </Link>
                    <button @click="showDeleteBuildingModal = true"
                        ...
                        {{ t('buildings_edit.header.delete') }}
                    </button>
                    <button @click="submitSettings"
                        ...
                        {{ settingsForm.processing ? t('buildings_edit.header.saving') : t('buildings_edit.header.save_changes') }}
                    </button>
                </div>
            </div>
        </template>
        ...
        <div class="py-6">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div v-if="buildings.length > 1" class="flex items-center gap-2 overflow-x-auto pb-4 mb-4">
                    <span class="text-xs font-medium text-gray-400 uppercase tracking-wider me-2">{{ t('buildings_edit.wings.label') }}</span>
                    <Link
                ...
                <div v-if="buildings.length > 1"
                    ...
                    <span>{{ t('buildings_edit.wings.shared_notice') }}</span>
                </div>
                    ...
                    <div class="p-4 sm:p-6">
                        ...
                        <div v-show="activeTab === 'units'" class="space-y-4">
                            ...
                            <div class="flex flex-wrap gap-3 items-center justify-between">
                                <div class="flex items-center gap-2">
                                    <div v-if="selectedIds.length > 0"
                                        ...
                                        <span class="text-sm font-semibold">{{ t('buildings_edit.units.selected_count', { count: selectedIds.length }) }}</span>
                                    ...
                                    <button @click="openActionModal('update_rent')"
                                        ...
                                        {{ t('buildings_edit.units.set_rent') }}
                                    </button>
                                    <button @click="openActionModal('update_type')"
                                        ...
                                        {{ t('buildings_edit.units.set_type') }}
                                    </button>
                                ...
                                <div class="flex items-center gap-2">
                                    <button @click="selectAll"
                                        ...
                                        {{ selectedIds.length === units.length ? t('buildings_edit.units.deselect_all') : t('buildings_edit.units.select_all') }}
                                    </button>
                                    <button @click="openAddUnitModal()"
                                        ...
                                        {{ t('buildings_edit.units.add_unit') }}
                                    </button>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center gap-4 text-xs text-gray-500">
                                <div class="flex items-center gap-1.5">
                                    ...
                                    {{ t('buildings_edit.legend.occupied') }}
                                </div>
                                <div class="flex items-center gap-1.5">
                                    <div class="w-2.5 h-2.5 rounded-full bg-gray-300"></div>
                                    {{ t('buildings_edit.legend.vacant') }}
                                </div>
                                <div class="flex items-center gap-1.5">
                                    <div class="w-2.5 h-2.5 rounded-full bg-amber-400"></div>
                                    {{ t('buildings_edit.legend.maintenance') }}
                                </div>
                                <div class="flex items-center gap-1.5">
                                    <div class="w-2.5 h-2.5 rounded-full bg-red-400"></div>
                                    {{ t('buildings_edit.legend.arrears') }}
                                </div>
                                <div class="flex items-center gap-1.5">
                                    <div class="w-2.5 h-2.5 rounded-full bg-purple-400"></div>
                                    {{ t('buildings_edit.legend.commercial') }}
                                </div>
                            </div>
                            ...
                            <div class="max-h-150 overflow-y-auto rounded-lg border border-gray-200">
                                <div v-for="floorGroup in unitsByFloor" :key="floorGroup.floor" class="border-b border-gray-100 last:border-b-0">
                                    ...
                                    <div class="px-4 sm:px-6 py-2 bg-gray-50 sticky top-0 z-10 flex items-center justify-between">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span class="text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ t('buildings_edit.units.floor', { floor: floorGroup.floor }) }}</span>
                                            <span class="text-xs text-gray-400">{{ t('buildings_edit.units.units_count', { count: floorGroup.units.length }) }}</span>
                                        </div>
                                        ...
                                        <div class="grid gap-2 sm:gap-3" :style="floorGridStyle">
                                            <button
                                                ...
                                                <span class="text-[10px] uppercase tracking-wide mt-0.5">
                                                    {{ unit.status === 'occupied' ? t('buildings_edit.units.status_abbr.occupied') : unit.status === 'arrears' ? t('buildings_edit.units.status_abbr.arrears') : unit.status === 'maintenance' ? t('buildings_edit.units.status_abbr.maintenance') : t('buildings_edit.units.status_abbr.vacant') }}
                                                </span>
                                ...
                                <div v-if="unitsByFloor.length === 0" class="p-12 text-center text-gray-400">
                                    ...
                                    <p class="text-sm font-medium">{{ t('buildings_edit.units.empty_title') }}</p>
                                    <p class="text-xs mt-1">{{ t('buildings_edit.units.empty_hint') }}</p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="space-y-5">
                                    ...
                                    <div>
                                        ...
                                        <select id="settings-currency" v-model="settingsForm.currency"
                                            ...
                                            <option value="USD">{{ t('buildings_edit.settings.currency_usd') }}</option>
                                            <option value="EUR">{{ t('buildings_edit.settings.currency_eur') }}</option>
                                            <option value="GBP">{{ t('buildings_edit.settings.currency_gbp') }}</option>
                                        </select>
                                        <p class="mt-1 text-xs text-gray-500">{{ t('buildings_edit.settings.currency_hint') }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="space-y-5">
                                    <h3 class="font-semibold text-gray-900 flex items-center gap-2">
                                        ...
                                        {{ t('buildings_edit.settings.structure_overview') }}
                                    </h3>
                                    <div class="grid grid-cols-2 gap-4">
                                        <div class="p-4 rounded-xl border border-gray-200 bg-gray-50">
                                            <div class="text-2xl font-bold text-gray-900">{{ building.total_floors }}</div>
                                            <div class="text-sm text-gray-500">{{ t('buildings_edit.settings.floors') }}</div>
                                        </div>
                                        <div class="p-4 rounded-xl border border-gray-200 bg-gray-50">
                                            <div class="text-2xl font-bold text-gray-900">{{ units.length }}</div>
                                            <div class="text-sm text-gray-500">{{ t('buildings_edit.settings.units') }}</div>
                                        </div>
                                        <div class="p-4 rounded-xl border border-green-200 bg-green-50">
                                            <div class="text-2xl font-bold text-green-700">
                                                {{ units.filter(u => u.status === 'occupied').length }}
                                            </div>
                                            <div class="text-sm text-gray-500">{{ t('buildings_edit.settings.occupied') }}</div>
                                        </div>
                                        <div class="p-4 rounded-xl border border-gray-200 bg-gray-50">
                                            <div class="text-2xl font-bold text-gray-500">
                                                {{ units.filter(u => u.status === 'vacant').length }}
                                            </div>
                                            <div class="text-sm text-gray-500">{{ t('buildings_edit.settings.vacant') }}</div>
                                        </div>
                                    </div>

                                    <div class="pt-4 border-t border-gray-100">
                                        <div class="flex items-center justify-between text-sm mb-2">
                                            <span class="text-gray-500">{{ t('buildings_edit.settings.occupancy_rate') }}</span>
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="text-start">
                                            ...
                                            <p class="text-xs text-gray-500">
                                                {{ t('buildings_edit.amenities.selected_of', { selected: getSelectedCount(category, items), total: Object.keys(items).length }) }}
                                            </p>
                                        </div>
                                    </div>
                                    <div class="flex items-center gap-2">
                            ...
                            <div class="border border-gray-200 rounded-xl overflow-hidden">
                                <div class="px-4 sm:px-6 py-4 bg-gray-50 border-b border-gray-200">
                                    <h3 class="font-semibold text-gray-900 flex items-center gap-2">
                                        ...
                                        {{ t('buildings_edit.amenities.custom_title') }}
                                    </h3>
                                    <p class="text-xs text-gray-500 mt-1">{{ t('buildings_edit.amenities.custom_subtitle') }}</p>
                                </div>
                                <div class="p-4 sm:p-6">
                                    ...
                                    <p v-else class="text-sm text-gray-400 text-center py-4">{{ t('buildings_edit.amenities.custom_empty') }}</p>
                                </div>
                            </div>
                        ...
                        <div v-show="activeTab === 'location'" class="space-y-6">
                            <div>
                                <h3 class="font-semibold text-gray-900 flex items-center gap-2 mb-4">
                                    ...
                                    {{ t('buildings_edit.location.title') }}
                                </h3>
                                <p class="text-xs text-gray-500 mb-4">{{ t('buildings_edit.location.hint') }}</p>
                                <BuildingMap
                                ...
                                <div v-if="settingsForm.coordinates?.lat" class="mt-4 flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div class="text-sm font-medium text-gray-900">{{ t('buildings_edit.location.coordinates_set') }}</div>
                                    ...
                                    <button @click="settingsForm.coordinates = null"
                                        ...
                                        {{ t('buildings_edit.location.clear') }}
                                    </button>
                                </div>
                            </div>
                            ...
                            <div v-if="canAccessWater" class="p-6 rounded-xl border border-blue-200 bg-blue-50">
                                <div class="flex items-start gap-4">
                                    ...
                                    <div class="flex-1">
                                        <h3 class="font-semibold text-gray-900">{{ t('buildings_edit.location.water_title') }}</h3>
                                        <p class="text-sm text-gray-600 mt-1">
                                            {{ t('buildings_edit.location.water_subtitle') }}
                                        </p>
                                        <Link :href="route('buildings.water-settings', building.id)"
                                            ...
                                            {{ t('buildings_edit.location.water_cta') }}
                                        </Link>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div v-show="activeTab === 'automation'" class="space-y-6">
                            <h3 class="font-semibold text-gray-900 flex items-center gap-2">
                                ...
                                {{ t('buildings_edit.automation.title') }}
                            </h3>
                            ...
                            <div class="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div class="font-medium text-gray-900">{{ t('buildings_edit.automation.auto_generate') }}</div>
                                        <div class="text-sm text-gray-500">{{ t('buildings_edit.automation.auto_generate_desc') }}</div>
                                    </div>
                                </div>
                            ...
                            <div v-if="automationForm.auto_generate_invoices" class="space-y-4">
                                <div>
                                    ...
                                    <p class="mt-1 text-xs text-gray-500">{{ t('buildings_edit.automation.generation_day_hint') }}</p>
                                </div>
                                ...
                                <div class="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div class="font-medium text-gray-900">{{ t('buildings_edit.automation.auto_send') }}</div>
                                            <div class="text-sm text-gray-500">{{ t('buildings_edit.automation.auto_send_desc') }}</div>
                                        </div>
                                    </div>
                                    <label class="relative inline-flex items-center cursor-pointer">
                            ...
                            <div class="bg-blue-50 border border-blue-200 rounded-xl p-4">
                                <div class="flex gap-3">
                                    ...
                                    <div class="text-sm text-blue-700">
                                        <p class="font-medium mb-1">{{ t('buildings_edit.automation.included_title') }}</p>
                                        <ul class="list-disc list-inside space-y-1 text-blue-600">
                                            <li>{{ t('buildings_edit.automation.included_rent') }}</li>
                                            <li>{{ t('buildings_edit.automation.included_water') }}</li>
                                            <li>{{ t('buildings_edit.automation.included_arrears') }}</li>
                                            <li>{{ t('buildings_edit.automation.included_late_fees') }}</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="flex justify-end pt-4 border-t border-gray-100">
                                <button @click="submitAutomation"
                                    ...
                                    <CheckIcon class="w-4 h-4 me-2" />
                                    {{ automationForm.processing ? t('buildings_edit.automation.saving') : t('buildings_edit.automation.save') }}
                                </button>
                            </div>
                        </div>
                        ...
                        <div v-show="activeTab === 'deductions'" class="space-y-6">
                            <h3 class="font-semibold text-gray-900 flex items-center gap-2">
                                <DocumentTextIcon class="w-5 h-5 text-gray-400" />
                                {{ t('buildings_edit.deductions.title') }}
                            </h3>
                            <p class="text-sm text-gray-500">{{ t('buildings_edit.deductions.subtitle') }}</p>

                            <div class="bg-blue-50 border border-blue-200 rounded-xl p-4">
                                <div class="flex gap-3">
                                    <DocumentTextIcon class="w-5 h-5 text-blue-500 shrink-0 mt-0.5" />
                                    <div class="text-sm text-blue-700">
                                        <p class="font-medium mb-1">{{ t('buildings_edit.deductions.about_title') }}</p>
                                        <ul class="list-disc list-inside space-y-1 text-blue-600">
                                            <li>{{ t('buildings_edit.deductions.about_always_apply') }}</li>
                                            <li>{{ t('buildings_edit.deductions.about_scope') }}</li>
                                            <li>{{ t('buildings_edit.deductions.about_adjust') }}</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="flex justify-center py-4">
                                <Link
                                    ...
                                    {{ t('buildings_edit.deductions.manage_cta') }}
                                </Link>
                            </div>
                        </div>
                            ...
                            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h3 class="font-semibold text-gray-900">
                                                {{ modalType === 'update_rent' ? t('buildings_edit.action_modal.set_rent_title') : t('buildings_edit.action_modal.set_type_title') }}
                                            </h3>
                                            <p class="text-xs text-gray-500">{{ t('buildings_edit.action_modal.applying_to', { count: selectedIds.length }) }}</p>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div v-if="modalType === 'update_type'">
                                        ...
                                        <div class="grid grid-cols-2 gap-3">
                                            ...
                                            <label :class="[
                                                ...
                                                <span :class="['font-medium', actionForm.value === 'commercial' ? 'text-purple-700' : 'text-gray-700']">{{ t('buildings_edit.unit_type.commercial') }}</span>
                                            </label>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="px-6 py-4 bg-gray-50 flex justify-end gap-3">
                                    <button @click="showActionModal = false"
                                        ...
                                        {{ t('buildings_edit.common.cancel') }}
                                    </button>
                                    <button @click="submitAction"
                                        ...
                                        {{ actionForm.processing ? t('buildings_edit.action_modal.applying') : t('buildings_edit.action_modal.apply_changes') }}
                                    </button>
                                </div>
                            </div>
                        </Transition>
                    ...
                    <div class="flex min-h-full items-center justify-center p-4">
                        ...
                        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
                            <div class="p-6 text-center">
                                ...
                                <h3 class="text-lg font-semibold text-gray-900 mb-2">{{ t('buildings_edit.delete_units.title') }}</h3>
                                <p class="text-sm text-gray-500">
                                    {{ t('buildings_edit.delete_units.body_prefix') }} <span class="font-semibold">{{ selectedIds.length }}</span> {{ t('buildings_edit.delete_units.body_suffix') }}
                                </p>
                            </div>
                            <div class="px-6 py-4 bg-gray-50 flex gap-3">
                                ...
                                <button @click="confirmDelete"
                                    ...
                                    {{ actionForm.processing ? t('buildings_edit.delete_units.deleting') : t('buildings_edit.delete_units.delete') }}
                                </button>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
                            <div class="px-6 py-4 border-b border-gray-100">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 class="font-semibold text-gray-900">{{ t('buildings_edit.add_unit.title') }}</h3>
                                        <p class="text-xs text-gray-500">{{ t('buildings_edit.add_unit.subtitle') }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <form @submit.prevent="submitAddUnit" class="p-6 space-y-5">
                                ...
                                <div>
                                    ...
                                    <div class="grid grid-cols-2 gap-3">
                                        ...
                                        <label :class="[
                                            ...
                                            <span :class="addUnitForm.unit_type === 'commercial' ? 'text-purple-700' : 'text-gray-700'">{{ t('buildings_edit.unit_type.commercial') }}</span>
                                        </label>
                                    </div>
                                </div>
                                ...
                                <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                                    ...
                                    <button type="submit"
                                        ...
                                        {{ addUnitForm.processing ? t('buildings_edit.add_unit.creating') : t('buildings_edit.add_unit.create') }}
                                    </button>
                                </div>
                            </form>
                        </div>
                    ...
                    <div class="flex min-h-full items-center justify-center p-4">
                        ...
                        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
                            <div class="p-6 text-center">
                                ...
                                <h3 class="text-lg font-semibold text-gray-900 mb-2">{{ t('buildings_edit.delete_building.title') }}</h3>
                                <p class="text-sm text-gray-500">
                                    <template v-if="building.is_wing && parentBuilding">
                                        {{ t('buildings_edit.delete_building.wing_prefix') }} <span class="font-semibold">{{ parentBuilding.name }}</span>{{ t('buildings_edit.delete_building.wing_suffix', { count: buildings.length }) }}
                                    </template>
                                    <template v-else>
                                        {{ t('buildings_edit.delete_building.single_prefix') }} <span class="font-semibold">{{ building.name }}</span> {{ t('buildings_edit.delete_building.single_suffix', { count: units.length }) }}
                                    </template>
                                </p>
                            ...
                            <div class="px-6 py-4 bg-gray-50 flex gap-3">
                                ...
                                <button @click="confirmDeleteBuilding"
                                    ...
                                    class="flex-1 px-4 py-2.5 text-sm font-medium text-white bg-red-600 rounded-lg hover:bg-red-700 transition-colors disabled:opacity-50">
                                    {{ deletingBuilding ? t('buildings_edit.delete_building.deleting') : t('buildings_edit.delete_building.delete') }}
                                </button>
                            </div>
                        </div>
                    </div>
