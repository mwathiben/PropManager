...
<template>
    <Head :title="t('vendor_portal.sla.title')" />
    <VendorPortalLayout :vendor-name="vendor.name">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <h1 class="text-xl font-semibold text-gray-900">{{ t('vendor_portal.sla.title') }}</h1>
        ...
        <div v-else class="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div class="flex items-center gap-5 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-100" data-testid="vendor-sla-within">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500">{{ t('vendor_portal.sla.within_sla') }}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-100">
                    ...
                    <p class="text-sm text-gray-500">{{ t('vendor_portal.sla.breached') }}</p>
                </div>
                <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-100">
                    <p class="text-2xl font-bold text-amber-600">{{ metrics.open_overdue }}</p>
                    <p class="text-sm text-gray-500">{{ t('vendor_portal.sla.open_overdue') }}</p>
                </div>
                <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-100">
                    <p class="text-2xl font-bold text-gray-900">{{ metrics.total_resolved }}</p>
                    <p class="text-sm text-gray-500">{{ t('vendor_portal.sla.resolved_total') }}</p>
                </div>
                <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-100">
                    <p class="text-2xl font-bold text-gray-900">
                        {{ metrics.avg_resolution_hours === null ? '—' : metrics.avg_resolution_hours + t('vendor_portal.sla.hours') }}
                    </p>
                    <p class="text-sm text-gray-500">{{ t('vendor_portal.sla.avg_resolution') }}</p>
                </div>
            </div>
        </div>
    </VendorPortalLayout>
