        ...
        <div class="py-6">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <table v-else class="min-w-full divide-y divide-gray-200 rounded-lg border border-gray-200 bg-white shadow-sm">
                    <thead>
                        <tr class="bg-gray-50">
                            <th class="px-3 py-2 text-start text-xs font-medium uppercase tracking-wide text-gray-500">Key</th>
                            <th class="px-3 py-2 text-start text-xs font-medium uppercase tracking-wide text-gray-500">Name</th>
                            <th class="px-3 py-2 text-start text-xs font-medium uppercase tracking-wide text-gray-500">Status</th>
                            <th class="px-3 py-2 text-end text-xs font-medium uppercase tracking-wide text-gray-500">Exposures</th>
                            <th class="px-3 py-2 text-start text-xs font-medium uppercase tracking-wide text-gray-500">Variants</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <tr v-for="exp in experiments" :key="exp.id">
                            ...
                            <td class="px-3 py-2 text-end text-sm text-gray-700">{{ exp.exposures_total }}</td>
                            <td class="px-3 py-2 text-sm text-gray-600">
                                <span v-for="(count, variantKey) in exp.exposures_by_variant" :key="variantKey" class="me-2">
                                    {{ variantKey }}: {{ count }}
                                </span>
                            </td>
                            <td class="px-3 py-2 text-end text-sm">
                                ...
                                <button
                                    ...
                                    type="button"
                                    class="ms-3 text-green-700 hover:underline"
                                    @click="flipStatus(exp.id, 'running')"
                                >Run</button>
                                <button
                                    v-if="exp.status === 'running'"
                                    type="button"
                                    class="ms-3 text-yellow-700 hover:underline"
