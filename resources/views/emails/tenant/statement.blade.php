...
|:-----|:-------------|---:|---:|---:|
@foreach ($rows as $row)
| {{ $row['date'] }} | {{ str_replace('|', '/', (string) $row['description']) }} | {{ $row['charge'] > 0 ? number_format($row['charge'], 2) : '' }} | {{ $row['payment'] > 0 ? number_format($row['payment'], 2) : '' }} | {{ number_format($row['running_balance'], 2) }} |
@endforeach
</x-mail::table>
